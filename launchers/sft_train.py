...
# Mirrors launchers/grpo_train.py and launchers/cross_encoder_train.py experiment definitions.
EXPERIMENTS = [
    # # Main models
    # {
    ...
    # },
    {
        ...
        "gpus": 2,
        "overrides": [
            "trainer_config.per_device_train_batch_size=2",
            "trainer_config.gradient_accumulation_steps=16",
...
def build_command(seed: int, exp: dict) -> str:
    output_dir = f"out/train/sft/seed_{seed}"
    override_str = " \\\n    ".join(exp["overrides"])

    return (
        f"gbatch --gpus {exp['gpus']} -- \\\n"
        f"uv run python3 scripts/sft_train.py \\\n"
